...
/**
 * Retrieves longest win streak for a team filtered between two dates from the API-database
 * @param {number} id
...
export default async function WinStreakByTeam(id, from, to, cb) {
  axios
    .get(
      `/game/longest_winning_streak/team_id/${id}/start_date/${from}/end_date/${to}`
    )
    .then(res => {
      cb(res.data.length);
    })
    .catch(e => {
      console.log("Failed to retrieve winning streak for id: " + id);
    });
}
